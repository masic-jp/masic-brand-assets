...
- `business-card/_base/*.html` は **`_rebuild.py` の入力スケルトン**（中立プレースホルダ入りの commit 済 standalone）。基本的に直接編集しない。Anthropic Design Canvas で再生成した standalone がある場合のみ手動で差し替える
